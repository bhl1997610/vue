...
def wxinit():
    ...
    findchannel()
    tm = threading.Timer(1200, wxinit)
    tm.start()
    ...
    try:
        ...
        if j == len(cols)-1:
            for i in range(len(info)):
                mysql.addMPDB(info[i].values()[0], info[i].values()[1],info[i].values()[2],"干瞪眼", info[i].values()[
...
time.sleep(2)
# img.screenshot(".\\public\\images\\"+load_dict[sys.argv[1]]+".png")
img.screenshot(".\\public\\images\\bb.png")
